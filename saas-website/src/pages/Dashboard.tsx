import { Activity, FolderOpen, Search, Settings, Link as LinkIcon, Clock, Copy, ExternalLink, Edit2, Lock, Mail, MessageCircle, LogOut, Trash2, MoreVertical, Share2, Download } from 'lucide-react';
    ...
    <div className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 font-['Space_Grotesk'] text-white">
        ...
        <div className="mt-6 flex flex-col sm:flex-row md:ml-4 md:mt-0 gap-4">
          ...
          {!extensionDetected && (
            <a 
              ...
              className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-4 py-2 sm:px-5 sm:py-2.5 text-xs sm:text-sm font-bold text-gray-900 shadow-xl hover:bg-gray-200 transition-all active:scale-95 no-underline pointer-events-auto"
            >
      ...
      <div className="bg-white/5 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white/10 relative z-10">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 sm:py-28 px-6">
            <div className="mx-auto h-24 w-24 sm:h-28 sm:w-28 rounded-[2rem] bg-white/5 flex items-center justify-center mb-6 shadow-2xl border border-white/10 rotate-3 hover:rotate-0 transition-transform duration-300">
              <FolderOpen className="h-10 w-10 sm:h-12 sm:w-12 text-gray-500" />
            </div>
            <h3 className="mt-2 text-xl sm:text-2xl font-black text-white tracking-tight">
              {extensionDetected ? 'No matched forms' : 'Extension Setup Required'}
            </h3>
            <p className="mt-3 text-sm sm:text-base text-gray-400 max-w-lg mx-auto font-medium leading-relaxed">
              {extensionDetected 
                ? "You haven't generated any timer links yet or your search doesn't match."
                : "To start generating timed exam links, you first need to download and load our Chrome Extension in your browser."}
            </p>
            
            {!extensionDetected && (
              <div className="mt-8 max-w-md mx-auto bg-[#0E0E12]/80 backdrop-blur-md border border-white/10 rounded-3xl p-6 sm:p-8 text-left shadow-2xl">
                <h4 className="text-sm font-bold text-purple-400 uppercase tracking-widest mb-4">Quick Extension Setup</h4>
                <ol className="list-decimal list-inside space-y-3 text-xs sm:text-sm text-gray-300 font-medium leading-relaxed">
                  <li>Download the pre-compiled <a href="/extension.zip" download="google-forms-timer-extension.zip" className="text-purple-300 hover:text-purple-200 hover:underline font-bold inline-flex items-center gap-1 pointer-events-auto"><Download className="w-3.5 h-3.5 inline" /> extension.zip</a></li>
                  <li>Unzip the downloaded folder on your computer</li>
                  <li>Open browser extensions page and enable <strong>Developer mode</strong></li>
                  <li>Click <strong>Load unpacked</strong> and select the unzipped folder</li>
                  <li>Open any Google Form page and click the extension icon to start!</li>
                </ol>
                <div className="mt-8 flex flex-col sm:flex-row gap-3">
                  <a 
                    href="/extension.zip" 
                    download="google-forms-timer-extension.zip"
                    className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-white text-gray-900 px-4 py-2.5 text-xs font-bold shadow-xl hover:bg-gray-200 transition-all active:scale-95 no-underline pointer-events-auto"
                  >
                  ...
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-white/5 border border-white/10 text-white px-4 py-2.5 text-xs font-bold hover:bg-white/10 transition-all active:scale-95 no-underline pointer-events-auto"
                  >
                    View on GitHub
                  </a>
                </div>
