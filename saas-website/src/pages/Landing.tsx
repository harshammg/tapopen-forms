import { ShieldAlert, Timer, Zap, ChevronRight, HelpCircle, CheckCircle2, Shield, EyeOff, Sparkles, Download } from 'lucide-react';
...
export default function Landing() {
  ...
  const faqs = [
    ...
    {
      q: "Are my form configurations and data secure?",
      a: "Yes. All form configurations and user accounts are secured using database Row Level Security (RLS) on our cloud servers. Only you (the authenticated owner) can view, access, or modify your forms."
    }
  ];
  ...
  return (
    <div className="bg-[#0A0A0B] text-white min-h-screen selection:bg-purple-500/30 font-['Space_Grotesk'] overflow-hidden">
      ...
      {/* Quick Setup Guide */}
      <div className="py-20 sm:py-28 relative z-10 border-y border-white/5 bg-gradient-to-b from-transparent via-purple-950/5 to-transparent">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center mb-16">
            <h2 className="text-base font-bold leading-7 text-purple-400 tracking-widest uppercase">Quick Start Guide</h2>
            <p className="mt-3 text-3xl font-black tracking-tight text-white sm:text-5xl">Start in 3 Simple Steps</p>
            <p className="mt-4 text-base text-gray-400 font-medium">No local code setup, database configuration, or command-line run required. Just install and go.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Step 1 */}
            <div className="relative group bg-[#0E0E12]/80 backdrop-blur-md border border-white/10 rounded-3xl p-8 shadow-2xl hover:border-purple-500/30 transition-all duration-300 flex flex-col justify-between animate-fade-in">
              <div>
                <div className="absolute top-6 right-8 text-6xl font-black text-white/5 group-hover:text-purple-500/10 transition-colors duration-300">01</div>
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-purple-500/10 text-purple-400 mb-6 group-hover:scale-110 transition-transform duration-300">
                  <Download className="w-6 h-6" />
                </div>
                <h3 className="text-xl font-bold text-white mb-3">Download the Extension</h3>
                <p className="text-sm text-gray-400 leading-relaxed">
                  Download the pre-compiled extension ZIP archive directly from the website or access the official source repository on GitHub.
                </p>
              </div>
              <div className="mt-8 flex flex-col gap-2 relative z-20">
                <a href="/extension.zip" download="google-forms-timer-extension.zip" className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white text-gray-900 text-xs font-bold hover:bg-gray-200 transition-all active:scale-95 pointer-events-auto w-full text-center no-underline">
                  <Download className="w-3.5 h-3.5" /> Download ZIP
                </a>
                <a href="https://github.com/harshammg/tapopen-forms" target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs font-bold hover:bg-white/10 transition-all active:scale-95 pointer-events-auto w-full text-center no-underline">
                  View on GitHub
                </a>
              </div>
            </div>
            ...
            {/* Step 2 */}
            <div className="relative group bg-[#0E0E12]/80 backdrop-blur-md border border-white/10 rounded-3xl p-8 shadow-2xl hover:border-purple-500/30 transition-all duration-300 flex flex-col">
              <div>
                <div className="absolute top-6 right-8 text-6xl font-black text-white/5 group-hover:text-purple-500/10 transition-colors duration-300">02</div>
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-purple-500/10 text-purple-400 mb-6 group-hover:scale-110 transition-transform duration-300">
                  <ChromeIcon className="w-6 h-6" />
                </div>
                <h3 className="text-xl font-bold text-white mb-3">Load in Browser</h3>
                <p className="text-sm text-gray-400 leading-relaxed font-medium">
                  Extract the downloaded ZIP folder. Go to <code className="bg-white/5 px-1.5 py-0.5 rounded text-purple-300 text-xs font-mono">chrome://extensions/</code> in Chrome (or extension settings in Edge/Brave), enable <strong>Developer mode</strong> in the top right, and click <strong>Load unpacked</strong>. Select the unzipped folder.
                </p>
              </div>
            </div>
            ...
            <div className="relative group bg-[#0E0E12]/80 backdrop-blur-md border border-white/10 rounded-3xl p-8 shadow-2xl hover:border-purple-500/30 transition-all duration-300 flex flex-col">
              <div>
                <div className="absolute top-6 right-8 text-6xl font-black text-white/5 group-hover:text-purple-500/10 transition-colors duration-300">03</div>
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-purple-500/10 text-purple-400 mb-6 group-hover:scale-110 transition-transform duration-300">
                  <Timer className="w-6 h-6" />
                </div>
                <h3 className="text-xl font-bold text-white mb-3">Use on Google Forms</h3>
                <p className="text-sm text-gray-400 leading-relaxed font-medium">
                  Open any Google Form view/edit page. Click the Extension icon in your toolbar, set your exam duration, and generate the secure, timed exam link for your students. Manage all timed links from your dashboard.
                </p>
              </div>
            </div>
          </div>
